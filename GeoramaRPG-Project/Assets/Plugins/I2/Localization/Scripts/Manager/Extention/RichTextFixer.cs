...
namespace RTLTMPro
{
    public static class RichTextFixer
    {
        ...
        struct TagPair
        {
            public FastStringBuilder open, close;
        ...
        /// </summary>
        public static void Fix2(FastStringBuilder text)
        {
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                if (tag.Type == TagType.Closing && (stack.Count == 0 || stack.Peek().End != tag.End))
                {
                    stack.Push(tag);
                    continue;
                }
                ...
                if(tag.Type == TagType.Opening && (stack.Count > 0 && stack.Peek().Type == TagType.Closing))
                {
                    var lastTag = stack.Pop();
                    var openingBuffer = new FastStringBuilder(tag.End - tag.Start + 1);
                    var closingBuffer = new FastStringBuilder(lastTag.End - lastTag.Start + 1);
                    text.Substring(closingBuffer, lastTag.Start, lastTag.End - lastTag.Start + 1);
                    text.Substring(openingBuffer, tag.Start, tag.End - tag.Start + 1);
                    
                    tagPairs.Add(new TagPair(){open = openingBuffer, close = closingBuffer, startIndex = lastTag.Start});
                    i = tag.End;
                }
            }

            foreach (TagPair tagPair in tagPairs)
            {
                text.Replace(tagPair.open, tagPair.close, true, tagPair.startIndex);
                text.Replace(tagPair.close, tagPair.open, true, tagPair.startIndex);
            }
        }

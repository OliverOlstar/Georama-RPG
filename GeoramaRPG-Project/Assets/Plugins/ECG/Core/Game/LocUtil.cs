...
public static class LocUtil
{
	...
	public static bool IsDebugLoc => CoreDebugOptions.ForceDebugLoc.IsSet() && (!CoreDebugOptions.ForceDebugLoc.IsStringSet(out string language) || string.IsNullOrEmpty(language));
	public static string DebugLanguage
	{
		get
		{
			string rt = null;
			if (CoreDebugOptions.ForceDebugLoc.IsSet() && CoreDebugOptions.ForceDebugLoc.IsStringSet(out string language))
			{
				rt = language;
			}
